...
Compiler.prototype.ctuplelistorset = function(e, data, tuporlist) {
    ...
    if (tuporlist !== "set" && e.ctx._type === "Store") {
        if (hasStars) {
    ...
    } else if (tuporlist === "set" || e.ctx._type === "Load") {
...
Compiler.prototype.cslice = function (s) {
    ...
    var high;
    var low;
    Sk.asserts.assert(s._type === "Slice");
    if (Sk.__future__.python3) {
        ...
        // Python 2 distinguishes a[:] from a[::], even with a missing step.
        // The modern AST represents both steps as null; recover only the trailing
        // punctuation after the last bound, excluding any expressions containing ':'.
        const bound = s.upper || s.lower;
        const startLine = bound ? bound.end_lineno : s.lineno;
        const startColumn = bound ? bound.end_col_offset : s.col_offset;
        const tail = this.source.slice(startLine - 1, s.end_lineno);
        const encoder = new TextEncoder();
        const decoder = new TextDecoder();
        tail[tail.length - 1] = decoder.decode(encoder.encode(tail[tail.length - 1]).subarray(0, s.end_col_offset));
        tail[0] = decoder.decode(encoder.encode(tail[0]).subarray(startColumn));
        const colons = (tail.join("\n").replace(/#[^\n]*/g, "").match(/:/g) || []).length;
        const explicitStep = s.step !== null || colons >= (s.upper ? 1 : 2);
        low = s.lower ? this.vexpr(s.lower) : explicitStep ? "Sk.builtin.none.none$" : "new Sk.builtin.int_(0)";
        high = s.upper ? this.vexpr(s.upper) : explicitStep ? "Sk.builtin.none.none$" : "new Sk.builtin.int_(2147483647)";
        step = s.step ? this.vexpr(s.step) : "Sk.builtin.none.none$";
    }
...
Compiler.prototype.vslicesub = function (s) {
    if (s._type === "Slice") return this.cslice(s);
    if (s._type === "Tuple" && s.elts.some(elt => elt._type === "Slice")) return this.eslice(s.elts);
    return this.vexpr(s);
};
...
Compiler.prototype.cboolop = function (e) {
    ...
    var ifFailed;
    var jtype;
    Sk.asserts.assert(e._type === "BoolOp");
    if (e.op._type === "And") {
...
Compiler.prototype.cconstant = function (value) {
    ...
    switch (value.type) {
        ...
        case "bytes":
            if (Sk.__future__.python3) return this.makeConstant("new Sk.builtin.bytes([", Array.from(value.value).join(","), "])");
            return this.makeConstant("new Sk.builtin.str(", getJsLiteralForString(Array.from(value.value, c => String.fromCharCode(c)).join("")), ",true)");
        ...
        default: Sk.asserts.fail("Unknown constant " + value.type);
    }
};
...
Compiler.prototype.vexpr = function (e, data, augvar, augsubs) {
    ...
    switch (e._type) {
        ...
        case "BinOp":
            return this._gr("binop", "Sk.abstr.numberBinOp(", this.vexpr(e.left), ",", this.vexpr(e.right), ",'", e.op._type, "')");
        case "UnaryOp":
            return this._gr("unaryop", "Sk.abstr.numberUnaryOp(", this.vexpr(e.operand), ",'", e.op._type, "')");
        case "Lambda":
        ...
        case "Call":
            ...
            return result;
        case "Constant":
            return this.cconstant(e.value);
        case "Attribute":
            if (e.ctx._type !== "AugLoad" && e.ctx._type !== "AugStore") {
                val = this.vexpr(e.value);
            }
            mangled = e.attr;
        ...
        case "Subscript":
            switch (e.ctx._type) {
                ...
                case "Store":
                case "Del":
                    return this.vslice(e.slice, e.ctx._type, this.vexpr(e.value), data);
                case "AugStore":
                    // To be more correct, we shouldn't sattr() again if the in-place update worked.
                    // At the time of writing (26/Feb/2015), Sk.abstr.numberInplaceBinOp never returns undefined,
                    // so this will never *not* execute. But it could, if Sk.abstr.numberInplaceBinOp were fixed.
            ...
            break;
        case "Name":
            return this.nameop(e.id, e.ctx._type, data);
        case "List":
        ...
        default:
            Sk.asserts.fail("unhandled case " + e._type + " vexpr");
    }
};
...
Compiler.prototype.vseqexpr = function (exprs, data) {
    ...
    // if (exprs.length === 1 && exprs[0]._type === "Starred") {
    //     exprs = exprs[0].value;
    // }
...
Compiler.prototype.caugassign = function (s) {
    ...
    var auge;
    var e;
    Sk.asserts.assert(s._type === "AugAssign");
    e = s.target;
    switch (e._type) {
        case "Attribute":
            to = this.vexpr(e.value);
            auge = {...e, ctx: {_type: "AugLoad"}};
            aug = this.vexpr(auge, undefined, to);
            val = this.vexpr(s.value);
            res = this._gr("inplbinopattr", "Sk.abstr.numberInplaceBinOp(", aug, ",", val, ",'", s.op._type, "')");
            auge.ctx = {_type: "AugStore"};
            return this.vexpr(auge, res, to);
        case "Subscript":
            // Only compile the subscript value once
            to = this.vexpr(e.value);
            augsub = this.vslicesub(e.slice);
            auge = {...e, slice: augsub, ctx: {_type: "AugLoad"}};
            aug = this.vexpr(auge, undefined, to, augsub);
            val = this.vexpr(s.value);
            res = this._gr("inplbinopsubscr", "Sk.abstr.numberInplaceBinOp(", aug, ",", val, ",'", s.op._type, "')");
            auge.ctx = {_type: "AugStore"};
            return this.vexpr(auge, res, to, augsub);
        case "Name":
            to = this.nameop(e.id, "Load");
            val = this.vexpr(s.value);
            res = this._gr("inplbinop", "Sk.abstr.numberInplaceBinOp(", to, ",", val, ",'", s.op._type, "')");
...
Compiler.prototype.exprConstant = function (e) {
    ...
    switch (v.type) {
        case "none": return 0;
        case "bool": case "str": case "float": return v.value ? 1 : 0;
        case "int": return v.value == 0 ? 0 : 1;
        case "bytes": return v.value.length ? 1 : 0;
        case "complex": return v.real || v.imag ? 1 : 0;
        default: return -1;
    }
};
...
Compiler.prototype.cfromimport = function (s) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        //out("print(\"getting Sk.abstr.gattr(", mod, ",", JSON.stringify(alias.name), ")\");");
...
Compiler.prototype.buildcodeobj = function (n, coname, decorator_list, args, callback, class_for_super) {
    ...
    //
    if (kwarg) {
        this.u.localnames.push(kwarg.arg);
        this.u.varDeclsCode += kwarg.arg + "=new Sk.builtins['dict']($kwa);";
...
Compiler.prototype.maybeCDocstringOfBody = function(body) {
    ...
    const stmt_0 = body[0];
    if (stmt_0._type !== "Expr")
        return null;

    const expr = stmt_0.value;
    if (expr._type !== "Constant" || expr.value.type !== "str")
        return null;
...
Compiler.prototype.cDocstringOfCode = function(node) {
    ...
    default:
        Sk.asserts.fail(`unexpected node kind ${node._type}`);
    }
}
...
Compiler.prototype.clambda = function (e) {
    var func;
    Sk.asserts.assert(e._type === "Lambda");
...
Compiler.prototype.vstmt = function (s, class_for_super) {
    ...
    switch (s._type) {
        ...
        case "Nonlocal":
            break;
        case "Expr":
            if (s.value._type === "Name" && s.value.id === "debugger") {
                out("debugger;");
                break;
            }
            this.vexpr(s.value);
...
Compiler.prototype.cprint = function (s) {
    ...
    var n;
    var dest;
    Sk.asserts.assert(s._type === "Print");
    dest = "null";
    if (s.dest) {
